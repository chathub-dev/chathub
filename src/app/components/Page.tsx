...
const PagePanel: FC<PropsWithChildren<{ title: string }>> = (props) => {
  return (
    <div className="flex flex-col overflow-hidden bg-white dark:bg-gray-800 rounded-[35px] h-full">
      <div className="text-center border-b border-solid border-[var(--border-1)] flex flex-col justify-center mx-10 py-3">
        <span className="font-semibold text-[var(--text-1)] text-lg">{props.title}</span>
      </div>
      <div className="mx-10 h-full overflow-auto pl-[2px] scrollbar">{props.children}</div>
    </div>
  )
}
